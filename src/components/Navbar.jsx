...
const Navbar = () => {
  ...
  return (
    <header className=" w-full sticky top-0 bg-white z-10 shadow-sm">
      <nav className=" relative h-16 flex justify-between items-center w-11/12 m-auto">
        ...
        <div className="md:hidden" onClick={showMobileMenu}>
          ...
          {/* mobile menu..... */}
          {nav && (
            <div className=" absolute top-16 flex-col left-0 w-full bg-cyan grid place-items-center text-center rounded-lg p-4 z-10 ">
              <ul className=" w-full">
                {navlinks.map((navlink, index) => (
                  <NavLink key={index} to={navlink.ref}>
                    <li
